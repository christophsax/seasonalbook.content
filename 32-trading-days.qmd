...
We build upon the case study in @sec-holidays where the retail sales was used. This type of series is also a candidate since retail sales usually shows differences between weekdays and weekend. We verify this assumption adding a `td1coef` regressor.
...
m0 <- seas(
  x = trade_tur,
  regression.variables = c("ls2008.2", "ls2009.1", "ao2015.2"),
...
m1 <- seas(
  x = trade_tur,
  regression.variables = c("ls2008.2", "ls2009.1", "ao2015.2", "td1coef"),
  arima.model = "(1 1 0)(0 1 0)",
  regression.aictest = NULL,
  outlier = NULL,
  transform.function = "log"
...
summary(m1) 
```
Eventually, the `td1coef` regressor is not significant.

## Case Study: Azerbaijani Consumer price index for construction

`td1coef` is significant.
...
1. Use the `trade_tur` series. Try adding a `td1coef` regressor to model the difference between weekday and weekend. Is that significant? What happens if you model simultaneously the holiday and trading days effect? Do you get better results?
