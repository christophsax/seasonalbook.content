...
Similar to the holiday adjustments, X-13 offers various tools to deal with differences in the number of days or weekdays. This chapter discusses the automated trading day adjustemts in X-13, and explores various ways of user customization. Note that the term *trading day* adjustmen is X-13 slang for weekday adjustment and will be used interchangeably.
...
## Case Study: Replicate X-13 trading days adjustment
...
For each month, `td_m_tbl` computes the number of specific weekdays, and compares them to the number of Sundays (`wd == -`) (columns `mon` to `sat`). For example, in May 1931, the number Mondays was 4 while the number of Sundays was 5. The column `td1` compares the total number of weekdays with the total number of weekends. The normaization formula is the one used by X-13 and described in the X-13 manual.
...
It is straigtforward to replicate these values to the automated procedure in X-13:
...
The built-in trading day adjustment in X-13 is exclusively based on the calendar to construct regressor variables, meaning it does not take into account country-specific holidays like Thanksgiving in the US. These holidays are addressed separately, as discussed in the @sec-holidays section.
