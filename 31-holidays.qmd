...
In seasonal adjusmtent, the treatment of Christmas is usually not much of a problem, because Christmas always occur on the same date in Decemeber.
...
Because their occurence follow different rules, the holiday happens at different months in different years.
...
: Values for `regression.variables` that are relvant for holiday adjustment {#tbl-regression-arguments tbl-colwidths="\[20,80\]"}
...
This opens the question why the one day model has been chosen in the beginning.
...
Let's build a easter regressor, using `genhol()`:
...
ramadan_ts <- seasonal::genhol(ramadan, frequency = 4, start = 0, end = 29, center = "calendar")
...
# Applying the calendar mean adjustment
ramadan_pre_ts <- seasonal::genhol(ramadan, frequency = 4, start = -2, end = 0, center = "calendar")
library(tsbox)
m4 <- seas(
  trade_tur,
...
# Q: The signs feel intuitively wrong?
summary(m4)
