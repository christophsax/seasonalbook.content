...
As we have seen in chapter FIXME, regARIMA consists of several interacting specs.
Some of the these specs will be further detailed in later chapters (Outlier, Trading Days).

Like most statistical modeling in X-13, the process to find the best model is iterative.
...
Once a satisfiying regARIMA has been found, one proceeds to the seasonal adjustment step.
...
However, it worth paying attention to correctly set the differencing and regression variables for seasonal adjustment.
...
Therefore, first differencing is effective for a series with a linear trend, while second differencing works for a series with a quadratic trend.
...
[^23-regarima-3]: The "S" in SARIMA stands for seasonal.
    Since most of the ARIMA models within X-13 are seasonal, we will restrain from using the term and usueally refer to an ARIMA model of the structure (p, d, q)(P, D, Q).
...
In many cases, the automated modeling capabilities of X-13 are sufficient for determining a suitable model.
This auto-generated model can also act as an starting point for further refinement of the regARIMA model.
