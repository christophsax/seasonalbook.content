...
-   Case Study: Changing the length of trend and/or seasonal filter
 ...
 -->
```
The fundamental task of seasonal adjustment is to decompose a series into its constituent components: a trend, a seasonal component, and an irregular component.
...
X-11 is a non-parametric procedure that works by passing predefined moving-average filters over the data.
SEATS, on the other hand, derives the moving-average filters from the underlying ARIMA model and will be discussed in @sec-seats.
...
To deepen our understanding of the X-11 method, we start with a "by-hand" approach in R.
...
If a values is larger than the larger limit, it is fully replaced. If a value is smaller than the lower limit, it is not replaced. If a value lies between the limits, it a weigted average between the two surrounding values and the value itself is used.

Note, that only non-replaced values are used to derive the replacement values.
Hence, if a very small limit is used, the values that are uesd to derive the replacement values may be far way.
...
An alternative way to inspect output of X-13 is to use the `out()` function, which accesses the X-13 HTML output that contains a pletora of infomation:
...
1.  Use *seasonal* to perform an automated seasonal adjusmtent of `fdeaths`, using X-11.

2. Use `monthplot()` to inspect the seasonal component.

3. Adjust the filter length to a longer filter (`s3x15`). Can you see the differences in `monthplot()`?

4. Use `out()` to inspect the extreme value adjustment. Which default values for `sigmalim` have been chosen?

5. Use lower values for `sigmalim` and see if it affects the results of `monthplot()`.
