...
As we have seen in @sec-additive-and-multiplicative-adjustment, we write the fundamental identity as:
...
Once the decomposition is done, the seasonally adjusted series for an additive[^24-x11-1] decomposition can be computed as:

[^24-x11-1]: For a multiplicative decomposition, the fundamental identity is
...
This basic seasonal adjustment is a useful tool to understand other features of X-13.
...
A variant of the simple decomposition performed in Section @sec-simple-decomposition is run 3 times along with a pre-processing step to fit a regARIMA model.
...
The X-11 spec also allows users to control the length of the trend and seasonal moving average filters used during the adjustment.
