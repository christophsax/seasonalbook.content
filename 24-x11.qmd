...
In seasonal adjustment, the fundamental task is to decompose a series into its constituent components: **trend (T_t)**, **seasonal (S_t)**, and **irregular (I_t)**. X-13ARIMA-SEATS offers two main techniques for this task: the X-11 method and SEATS.
...
X-11 is a nonparametric procedure that works by passing predefined moving-average filters over the data. (SEATS derives the moving-average filters from the underlying ARIMA model and will be discussed in in @sec-seats.)



As established in previous chapters and in @sec-additive-and-multiplicative-adjustment, the fundamental identity for an additive decomposition model is expressed as:

$$
X_t = T_t + S_t + I_t
$$

Here, $X_t$ represents the observed time series. The seasonally adjusted series for an additive decomposition is then calculated as:

$$
A_t = X_t - S_t
$$

For a multiplicative decomposition model, the identity and adjusted series are represented differently:
...
## Fundamental Identity of Seasonal Decomposition {#sec-seasonal-decomposition}

As established in @sec-additive-and-multiplicative-adjustment, the fundamental identity for an additive decomposition model is expressed as:
...
[^24-x11-1]: For a multiplicative decomposition model, the identity and adjusted series are represented differently:
