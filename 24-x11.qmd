...
The X-11 method is sensitive to outliers. In order to correct for these, X-11 applies a specialiced extreme value correction.
...
Users can modify the sigma thresholds (`sigmalim`) for extreme value conversion within the X-11 specification. This option, defined as a two-length vector that specifies when weighting should begin and when full weight of zero should be applied.
...
Extreme value choices can significantly influence the seasonal adjustment outcome. For example, the default values for in `AirPassengers` adjustment looks as follows:
