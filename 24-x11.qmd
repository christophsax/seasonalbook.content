...
-   Case Study: Changing the length of trend and/or seasonal filter
 ...
 -->
```
The fundamental task of seasonal adjustment is to decompose a series into its constituent components: a trend, a seasonal component, and a irregular component.
X-13 offers two main techniques for this task: the X-11 method and SEATS.
...
X-11 is a nonparametric procedure that works by passing predefined moving-average filters over the data.
SEATS, on the other hand, derives the moving-average filters from the underlying ARIMA model and will be discussed in in @sec-seats.
...
Both X-11 and SEATS achieve the decomposition by applying moving average filters over the series.
In the following, we describe the basic idea behind the seasonal decomposition in X-11.
If you want to perform a seasonal adjustment in R, run the following:
...
To estimate the seasonal component, a *seasonal filter* is applied to the detrended series.
...
The regARIMA model model also serves as the foundation of the initial steps in the X-11 algorithm to remove outliers and other regression effects before application of moving-averages.

#### Extreme Values {.unnumbered}

The X-11 method is sensitive to outliers.
In order to correct for these, X-11 applies a specialiced extreme value correction.
...
As a basic trend filter, we trend filter---a 2x12 moving average.
The weights for this filter are distributed as 1/24, followed by 1/12 for the next 11 points, and then 1/24 again.
We apply these weights to estimate the trend of the `AirPassengers` series:
...
If a user does not select a filter, the X-11 method defaults to automatic filter selection.
To understand the implications of filter length, consider the finite set of options available during an X-11 adjustment.
@tbl-x11-filters details the different filters that can be applied to both the seasonal and trend components of the series.
...
As mentioned above, X-11 is sensititve to outliers, and applies it's own extreme value adujstment (see Box for the differences between outliers and extrem values).
...
After an intitial trend estimation, X-11 replaces extreme values according to the following procedure:
...
flowchart LR
    A(  Estimate standard deviation <br> of irregular, sigma  )-->B(Compare SI-ration to <br> multiples of sigma)
    B-->C(SI smaller than 1.5 * sigma)
    B-->D(1.5 * s smaller than SI 2.5 * sigma)
    B-->E(SI larger than 2.5 * sigma)
    C --> F(SI unchanged)
    D --> G(SI linearly weighted)
    E --> H(SI fully weighted)
```

Users can modify the sigma thresholds (`sigmalim`) for extreme value conversion within the X-11 specification.
...
SI ratios are replaced with an average of the two nearest SI ratios from the same period.
...
For example, the default values for in `AirPassengers` adjustment looks as follows:
