...
For example, once a series is transformed it is usually passed to the `regression` and `arima` specs, which estimate an regARIMA model.
...
Once the series is modeled, it is decomposed either by the `seats` or by the `x11` spec.
...
In this book, we will list and explain the frequently used arguments, while skipping some of the more exotic ones.

## Addressing specs from R

In the R package seasonal, spec argument combinations can be directly fed to the `seas()` function.
For example, in order to turn off the log transformation in the `AirPassengers` example fron @sec-a-minimal-example, we can specify the following:
...
The `history` spec, for example, produces a sequence of runs from a sequence of truncated versions of the time series and allows the analysis of potential revisions.
The `slidingspans` models various parts of the time series and has a similar purpose as `history`.
...
The former enforces the yearly totals of the seasonally adjusted series to be equal those of the original series, while the latter allows a comparison of indirect and direct seasonal adjustments.
...
Some of them are vintage spec that were important in earlier versions of X-13, but were superseded by other specs.
It is now generally recommended to use `regression` instead of `x11regression` and `automdl` instead of `pickmdl`.
Other specs have a purely technical purpose.
The `series` spec, for example, provides X-13 with the data, the starting date and the frequency.
...
To perform a seasonal adjustment on `AirPassengers` with X11, we simply need to activate the `x11` spec.
...
If more than one mutually exclusive spec is included in `seas()`, specs are overwritten according the priority rules shown in @tbl-priority-rules
...
### Turning off auto modelling

By default, the `automdl` spec finds a good ARIMA model.
By specifying the `model` argument of `arima` spec, the automated modelling is deactivated.
...
### Turning off AIC testing and Outlier detection
...
In practice, many spec argument combinations can extracted via the `static()` functions, which will be demonstrated in the next chapter.
