...
We can put in a change of regime regression variable that allows for different seasonal regressors before and after January 2000. 
This regression variable is discussed in more detail below in @sec-modelling-seasonal-breaks.
...
## Modelling seasonal breaks {#sec-modelling-seasonal-breaks}
...
## F-test for seasonality when using change of regime regressor

We can conduct an F-test for seasonality similar to those discussed in @sec-f-test-seasonality. 
Here instead of a single F-test over the whole model span, instead we look at 3 different F-tests; before the change of regime, after the change of regime and a combined test.
We can find the F-test results by using the `grep` function over the `names(udg(m))` character vector. 

```{r}
ftest_udg_names = grep(pattern = "ftest", names(udg(m)), value = TRUE)
ftest = udg(m, ftest_udg_names) |> t() |> round(4)
colnames(ftest) = c("df1", "df2", "test_stat", "p_val")
ftest
```
